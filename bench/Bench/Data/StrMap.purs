...
benchStrMap = do
  ...
  benchFromDissectable = do
    let natStrs = show <$> L.range 0 99999
        natPairs = (flip Tuple) unit <$> natStrs
        shortPairList = L.take 10000 natPairs

    log $ "fromDissectable (" <> show (L.length shortPairList) <> ")"
    benchWith 100 \_ -> M.fromDissectable shortPairList

    log $ "fromDissectable (" <> show (L.length natPairs) <> ")"
    benchWith 10 \_ -> M.fromDissectable natPairs
